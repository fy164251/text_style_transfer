...
from utils import get_val_as_str, invert_dict, load_dataset, load_dataset2, load_model, load_turk_scores, merge_datasets
import numpy as np
import pandas as pd
import re
...
class UnigramBasedClassifier(NaturalnessClassifier):
    def __init__(self, style_transfer_model_name, text_vectorizer=TEXT_VECTORIZER):
        self.path = f'{CLASSIFIER_BASE_PATH}/unigram_{style_transfer_model_name}.pkl'
...
def display_agreements():
    ...
    human_absolute_judgments_for_input_texts = np.load(f'../evaluations/human/{ASPECT}/input_texts.npz')['absolute']

    for model in MODEL_TO_PARAMS:
        print(f'{model} AGREEMENTS:')
        print('====================')
        param_name = MODEL_TO_PARAM_NAMES[model]
        param_values = MODEL_TO_PARAMS[model]

        # load automated scores for input texts
        automated_scores_for_input_texts = np.load(f'{AUTOMATED_EVALUATION_BASE_PATH}/input_texts.npz')
        ...
        for val in param_values:
            ...
            automated_scores_for_output_texts_path = f'{AUTOMATED_EVALUATION_BASE_PATH}/{model}_{param_name}_{str_val}.npz'
        ...
        for a in agreements:
            ...
            print(f"{a.upper()}: {agreement_ratio}%")
        print()
...
donor = load_dataset2('/Users/gkohli/Desktop/SCPD/CS230/style-transfer-model-evaluation/transfer_model_outputs/GPT2/OneLinerRand.txt')
generated = load_dataset2('/Users/gkohli/Desktop/SCPD/CS230/style-transfer-model-evaluation/transfer_model_outputs/GPT2/OneLinerNabovokov.txt')
...
print(donor)
print(generated)
output_texts = merge_datasets(donor, generated)
